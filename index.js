...
// Nested function can access the variable internal because it is a closure - that is, it can essentially reach out / up to the scope of myFunction to access variables that are not found within its own scope.
...
const zooAnimals = [
  ...
  */
  ...
  const lowerCaseNamesArray = [];

  function lowerCaseNames(array){
    array.map((item) => {
      const index = array.indexOf(item);
      const lowerCaseName = array[index].animal_name.toLowerCase();
      lowerCaseNamesArray.push(lowerCaseName);
    });
  ...
  */
  ...
  const lowPopulationAnimalsArray = [];

  function lowPopulationAnimals(array){
    array.filter((item) => {
      const index = array.indexOf(item);
      if (array[index].population < 5) {
        lowPopulationAnimalsArray.push(item);
      }
    });
  ...
  */
  ...
  function USApop(array){
    const totalPopulation = array.reduce((accumulator, currentValue) => {
